...
// Roles allowed to see the in-app notification bell. Extends the original purchase-order
// audience (ea/md/purchase_manager) to the KIA sales chain so callback-request notifications
// reach the assigned sales person, their sales manager, the Sales GM, and the MD.
...
export function NotificationBell({ userId, userRole }: NotificationBellProps) {
  ...
  const fetchNotificationsData = useCallback(async (attempt = 1): Promise<{ notifications?: NotificationRowShape[]; unreadCount?: number }> => {
    if (!userId || !userRole || !NOTIFICATION_ROLES.includes(userRole)) {
  ...
  useEffect(() => {
    if (!userId || !userRole || !NOTIFICATION_ROLES.includes(userRole) || !('serviceWorker' in navigator)) {
      return
    }
  ...
  useEffect(() => {
    if (!userId || !userRole || !NOTIFICATION_ROLES.includes(userRole) || typeof window === 'undefined') {
      return
    }
