...
import { reopenSale } from '@/lib/h-promise/server'

export const dynamic = 'force-dynamic'

/** MD only: reopen an approved sale (approved → pending, back to the GSM / SM), with a reason. */
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const access = await requireHPromiseApi((caps) => caps.approvals.final)
  if (access.denied) return access.denied
  try {
    const { id } = await params
    const result = await reopenSale(actorOf(access.appUser), access.caps, id, await readJsonBody(request))
