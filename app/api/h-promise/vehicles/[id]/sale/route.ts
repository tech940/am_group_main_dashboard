...
import { saveSale } from '@/lib/h-promise/server'

export const dynamic = 'force-dynamic'

/** Record the sale, or edit the recorded one. Saving a rejected sale resubmits it. */
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const access = await requireHPromiseApi((caps) => caps.register.edit)
  if (access.denied) return access.denied
  try {
    const { id } = await params
    const result = await saveSale(actorOf(access.appUser), access.caps, id, await readJsonBody(request))
