...
import { decideSale } from '@/lib/h-promise/server'

export const dynamic = 'force-dynamic'

/** Approve or reject a sale — at the GSM / SM stage, or finally as the MD. Nobody decides a sale they entered. */
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const access = await requireHPromiseApi((caps) => caps.approvals.approve || caps.approvals.final)
  if (access.denied) return access.denied
  try {
    const { id } = await params
    const result = await decideSale(actorOf(access.appUser), access.caps, id, await readJsonBody(request))
