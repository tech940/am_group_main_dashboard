...
import { getPlatinumSalesReportSummary } from '@/lib/platinum/sales-report'
...
export async function GET(request: Request) {
  const timer = createApiTimer('platinum-sales-report-summary')
  ...
  try {
    ...
    const data = await timer.time('summary', () => getPlatinumSalesReportSummary({
      canViewPii: canViewPlatinumCustomerPii(appUser?.role),
      year: parsedYear,
      month: parsedMonth,
      monthKey,
    ...
    console.error('Failed to build Platinum sales report summary:', error)
    const timing = timer.finish()
    return withServerTiming(
      NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to load Platinum sales report summary' }, { status: 500 }),
      timing.serverTiming,
    )
  }
}
