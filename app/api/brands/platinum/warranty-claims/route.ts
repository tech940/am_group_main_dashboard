...
import {
  ...
  PLATINUM_WARRANTY_DEALER_GROUPS,
  getPlatinumWarrantyGroupForDealer,
} from '@/lib/platinum/warranty-dealers'
...
function sourceFrom(value: string | null): PlatinumWarrantySource {
  return value === 'ytp' ? 'ytp' : 'claim_list'
}

function permissionKey(source: PlatinumWarrantySource, action: 'view' | 'edit' | 'audit') {
  return `platinum.${source === 'ytp' ? 'warranty_list' : 'warranty_claim_list'}.${action}`
}
...
function dealerCodesForLocationKeys(locationKeys: string[]) {
  ...
  for (const key of locationKeys) {
    ...
    const group = PLATINUM_WARRANTY_DEALER_GROUPS.find(
...
function allowedDealerSqlFilter() {
  return sql`UPPER(TRIM(source_dealer_code)) IN (${sql.join(
    PLATINUM_WARRANTY_ALLOWED_DEALERS.map((code) => sql`${code}`),
    sql`, `,
  )})`
}

async function fetchSourceRows(source: PlatinumWarrantySource) {
...
async function fetchActionSummaries(source: PlatinumWarrantySource): Promise<ActionSummaries> {
...
function enrichRows(
  ...
  return rawRows.map((row) => {
    ...
    const matchingAction = requirement.required && requirement.code
      ...
      : true
    let dealerCode = text(row.source_dealer_code).toUpperCase() || 'UNMAPPED'
    // Map legacy Rajouri code to canonical Rajouri code
    if (dealerCode === 'N6824') {
      dealerCode = 'N6250'
    }
    return {
...
function warrantyCacheKey(source: PlatinumWarrantySource) {
  return platinumWarrantyBaseCacheKey(source)
}

async function loadWarrantyBase(source: PlatinumWarrantySource): Promise<WarrantyBaseCache & { scoped: EnrichedRow[] }> {
  const cached = await getCachedData(
    ...
    async () => {
      ...
      const dealers = [...new Set(rawRows.map((row) => {
        const code = text(row.source_dealer_code).toUpperCase()
        return code === 'N6824' ? 'N6250' : code
      }).filter(Boolean))].sort()
...
export async function GET(request: Request) {
  ...
  const matrix = source === 'claim_list' ? {
    ...
    groups: PLATINUM_WARRANTY_DEALER_GROUPS.map((group) => {
      const dealersInGroup = group.dealerCodes
        .map((code) => code === 'N6824' ? 'N6250' : code)
        .filter((code, idx, self) => self.indexOf(code) === idx) // Deduplicate Rajouri
  ...
  return NextResponse.json({
    ...
    options: {
      ...
      dealerOptions: dealers.map((code) => {
        ...
        const location = getPlatinumWarrantyGroupForDealer(code)?.label || null
        return {
          code,
      ...
      locationGroups: PLATINUM_WARRANTY_DEALER_GROUPS.map((group) => ({
        key: group.key,
        label: group.label,
