...
function sourceFrom(value: unknown): PlatinumWarrantySource {
  return String(value || '') === 'ytp' ? 'ytp' : 'claim_list'
}

function permissionKey(source: PlatinumWarrantySource, action: 'view' | 'edit') {
...
export async function GET(request: Request) {
  ...
  if (!isAllowedPlatinumWarrantyDealer(dealerCode)) {
    return NextResponse.json({ error: 'This dealer is not available in Platinum warranty' }, { status: 403 })
  }
  ...
  const actions = source === 'claim_list'
    ? resultRows(await db.execute(sql`
        SELECT a.id, a.requirement_code, a.status_snapshot, a.business_date_snapshot, a.remark,
          a.docket_number, a.created_by_name, a.created_by_role, a.created_at
  ...
  evidence.forEach((item) => {
    ...
    evidenceByAction.set(key, [...(evidenceByAction.get(key) || []), {
      ...
      previewUrl: `/api/brands/platinum/warranty-claims/evidence/${item.id}`,
    }])
  })
...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.error('Failed to save Platinum warranty action:', error)
