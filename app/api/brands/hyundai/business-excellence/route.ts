...
const BUSINESS_EXCELLENCE_TABLES = [
  { slug: 'hyundai_ro_billing_report', table: 'hyundai_ro_billing_report', sheetName: 'RO Billing Report' },
] as const
...
function roBillingWhereClause(startDate?: string | null, endDate?: string | null, dealerCode?: string | null) {
  ...
  if (dealerCodes.length > 0) {
    clauses.push(sql`UPPER(TRIM(COALESCE(NULLIF(dealer_code, ''), NULLIF(main_dealer_code, '')))) IN (${sql.join(dealerCodes.map((code) => sql`${code}`), sql`, `)})`)
  }
...
async function getRoBillingStats(table: BusinessExcellenceTable, startDate?: string | null, endDate?: string | null, dealerCode?: string | null) {
  return getCachedData(
    `${HYUNDAI_BE_CACHE_PREFIX}:ro-billing-stats:v1:${table.slug}:start:${startDate || 'none'}:end:${endDate || 'none'}:dealer:${dealerCode || 'all'}`,
    async () => {
...
async function getTableMetadata(table: BusinessExcellenceTable) {
  return getCachedData(
    `${HYUNDAI_BE_CACHE_PREFIX}:table-metadata:v1:${table.slug}`,
    async () => {
...
async function fetchTableRows({
  ...
  const useBillDateWindow = table.slug === 'hyundai_ro_billing_report' && startDate && endDate

  if (table.slug === 'hyundai_ro_billing_report' && fetchAll) {
  ...
  if (table.slug === 'hyundai_ro_billing_report') {
  ...
  const [metadata, rowsResult] = await Promise.all([
    ...
    table.slug === 'hyundai_ro_billing_report'
      ? db.execute(roBillingProjectedRowsSql(table, selectedLimit, selectedOffset, startDate, endDate, dealerCode))
      : useBillDateWindow
      ? db.execute(sql`
  ...
  return {
    ...
    page,
    limit: selectedLimit,
    rows: table.slug === 'hyundai_ro_billing_report' ? rowsResult : rowsResult.map((row) => row.row),
  }
}
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (brand !== 'hyundai') {
      return NextResponse.json({ error: 'Only Hyundai Business Excellence tables are configured' }, { status: 400 })
    }
    ...
    const cacheKey = `${HYUNDAI_BE_CACHE_PREFIX}:relational:metadata:Hyundai:ro_billing_only:v1`
