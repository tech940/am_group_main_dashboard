...
function complaintAttributeFilters(filters: ComplaintFilters) {
  return sql`
    ...
    ${complaintDealerCodeFilter(filters.dealerCode)}
...
export async function GET(request: Request) {
  ...
  try {
    ...
    timer.finish()
    console.error('Failed to build Hyundai complaints dashboard:', error)
    return NextResponse.json({ error: 'Failed to build Hyundai complaints dashboard' }, { status: 500 })
  }
}
