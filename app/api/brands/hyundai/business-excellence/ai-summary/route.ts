...
async function buildReportDataset(request: NextRequest, report: string, startDate: string, endDate: string, dealerCode?: string | null) {
  ...
  appendHyundaiDealerCodeParam(baseParams, dealerCode)

  if (report === 'Business Excellence Overview') {
    const data = await fetchJson(request, `/api/brands/hyundai/business-excellence/overview?${baseParams.toString()}`)
    return compactOverviewPayload(data)
  }

  if (report === 'Open RO (Repair Orders)') {
    const data = await fetchJson(request, `/api/brands/hyundai/business-excellence/open-ro?${baseParams.toString()}`)
    return compactOpenRoPayload(data)
  }

  if (report === 'Workshop Performance') {
    const data = await fetchJson(request, `/api/brands/hyundai/business-excellence/workshop-performance?${baseParams.toString()}`)
    return compactWorkshopPayload(data)
  }

  if (report === 'Hyundai Complaints') {
    const data = await fetchJson(request, `/api/brands/hyundai/business-excellence/complaints?${baseParams.toString()}`)
  ...
  appendHyundaiDealerCodeParam(roParams, dealerCode)
  ...
  appendHyundaiDealerCodeParam(intelligenceParams, dealerCode)

  const [table, trend, analytics, leaderboard, intelligence] = await Promise.all([
    fetchJson(request, `/api/brands/hyundai/business-excellence/ro-billing-analysis?${tableParams.toString()}`),
    fetchJson(request, `/api/brands/hyundai/business-excellence/ro-billing-analysis?${trendParams.toString()}`),
    fetchJson(request, `/api/brands/hyundai/business-excellence/ro-billing-analysis?${analyticsParams.toString()}`),
    fetchJson(request, `/api/brands/hyundai/business-excellence/ro-billing-analysis?${leaderboardParams.toString()}`),
    fetchJson(request, `/api/brands/hyundai/business-excellence/performance-intelligence?${intelligenceParams.toString()}`),
  ])
...
function createCacheKey(report: string, startDate: string, endDate: string, dataset: unknown, dealerCode?: string | null) {
  return `hyundai:business-excellence:ai-summary:v7:${createHash('sha1')
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const dealerCode = normalizeHyundaiDealerCode(body.dealerCode) || null
