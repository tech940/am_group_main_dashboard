...
import { getHyundaiSalesReportCsv, getHyundaiSalesReportTable } from '@/lib/hyundai/sales-report'
...
export async function GET(request: Request) {
  const timer = createApiTimer('hyundai-sales-report-reports')
  ...
  try {
    ...
    // The flag the query function has always accepted and nobody ever supplied. Derived from the
    // signed-in user's role, never from the request — a client-supplied value would be self-service.
    const canViewPii = canViewHyundaiCustomerPii(appUser?.role)
    ...
    if (url.searchParams.get('format') === 'csv') {
      const result = await timer.time('csv', () => getHyundaiSalesReportCsv(params))
      const timing = timer.finish()
    ...
    const data = await timer.time('table', () => getHyundaiSalesReportTable({
      ...params,
    ...
    console.error('Failed to read Hyundai sales report table:', error)
    const timing = timer.finish()
    return withServerTiming(
      NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to load Hyundai sales report table' }, { status: 500 }),
      timing.serverTiming,
    )
  }
}
