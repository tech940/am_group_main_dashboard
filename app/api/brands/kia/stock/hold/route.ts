...
import { holdKiaStockVehicle, releaseKiaStockHold, markKiaStockHoldPaymentReceived } from '@/lib/kia/bookings'
...
// #12 Hold a stock vehicle for a DEALER (action 'hold'), release a hold (action 'release'), or record
// payment against a hold within its 48h window (action 'payment'). Gated by kia.bookings.edit — same
// roles that allot/transfer.
export async function POST(request: Request) {
  try {
    ...
    if (action === 'payment') {
      const result = await markKiaStockHoldPaymentReceived(vinNumber, auth.appUser!)
      return NextResponse.json({ ok: true, ...result })
    }

    const result = await holdKiaStockVehicle(vinNumber, { notes: body?.notes ?? null }, auth.appUser!)
    return NextResponse.json({ ok: true, ...result })
  } catch (error) {
