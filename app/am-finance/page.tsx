...
import { canAccessAmFinance, canViewAmFinance, getAmFinancePermissions } from '@/lib/am-finance/access'
...
export default async function AmFinancePage() {
  ...
  // SHARED with all three /api/am-finance routes (lib/am-finance/access.ts). Stating it here and
  // role-only there is what let a denied user keep reading from the API.
  if (!(await canViewAmFinance(appUser))) {
    forbidden()
  }
