...
Business Excellence now opens each major section with Executive Layer V2.1 before detailed charts/tables. This layer must not duplicate AI Summary narration or show its own AI Summary button; it is a compact management command center with section-specific logic, score/status, previous score/change, positive and negative score drivers, target achievement, top driver, biggest concern, focus areas, and data confidence. Keep the UI sparse: one health card, one "what changed" card, a small decision KPI group, target achievement, and three action cards. RO Billing uses revenue/labour/parts/load/billing-quality/leakage metrics, Workshop uses labour/parts/VAS/WA/WB/revenue-mix metrics, Open RO uses WIP/aging/delayed/closure-pressure metrics, Complaints uses closure/open/aging/customer-risk metrics, and Demo Job Cards uses due/overdue/remarks/compliance metrics. Section card language must be purpose-built: revenue sections use revenue drivers/risks/opportunities, operational sections use operational drivers/risks/opportunities, customer sections use complaint/customer language, and demo uses due/compliance/follow-up language. AI Summary remains a separate modal for narrative Good News, Bad News, and Immediate Actions, launched from the main report controls only. RO Billing detailed navigation is simplified to Overview, Insights, Revenue, and People; the old Performance trend tab and admin performance diagnostics button/panel are removed from the Business Excellence shell.
...
- Demo Job Cards is a dedicated AM Kia sidebar module directly below Business Excellence at `/brands/kia/demo-job-cards`. It now works as a lightweight operational follow-up tracker, not an analytics dashboard. It uses only `ro_billing_report` rows where `work_type = 'Test Drive/CC Maintenance'`, dedupes to `1 vehicle = 1 row` using VIN with registration fallback, and picks the latest `bill_date` row per vehicle.
- Demo Job Cards next due logic is `next_demo_due_date = latest bill_date + 15 days`. Upcoming Demo Due alerts show vehicles where `next_demo_due_date - current_date <= 5`, sorted by closest due date first. The page intentionally removed KPI overload, aging charts, advisor charts, status charts, and monthly trend charts.
...
- The Demo Job Cards sidebar item opens in a new browser tab. Its API uses an exact `work_type = 'Test Drive/CC Maintenance'` predicate, only selects tracker columns needed for the operational table, caches vehicle lists for 75 minutes, and invalidates `kia:demo-job-cards:*` cache entries whenever remarks are added or edited.
