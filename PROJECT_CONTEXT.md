...
- Live Platinum schema contains 10 `am_platinum_%` tables: adv-wise lubricants/VAS, call-center complaints, demo cars, EW, operation-wise analysis, PSF, repair orders, RO billing, service appointments, and trust package. `am_platinum_mcp_report`, `am_platinum_rsa_report`, `am_platinum_operation_wise_analysis_advisor_report`, and `am_platinum_workshop_performance_jc_summary_v1` are not currently present; their metrics must remain unavailable rather than fabricated.
- Cache namespaces after this audit: Platinum Overview `v36`, RO Billing `v28` with base rows `v9`, Workshop Performance `v38`, Open RO `v13`, and Performance Intelligence `v12`.
