...
- Growth badges rendered inside black Business Excellence management rows (`MECH`, `MECH TOTAL`, `Grand Total`) must always use a solid white pill background. The global `be-management-total-row` CSS force-overrides translucent green/red/lab growth backgrounds to white so growth text remains readable in every table.
