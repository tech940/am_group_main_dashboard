...
- Hyundai Business Excellence now uses one canonical six-location dealer registry everywhere: Jammu `N5203/N5216`, Akhnoor `N5701/N6844`, Kathua `N5804/N6845`, RS Pura `N6815/N6846`, Vijaypur `N6819/N6847`, and Udhampur `N5217/N6848/N6849`. Legacy `ACTIVE` source rows map to Jammu only when no better dealer field exists. The obsolete `jammu=N5216/N6846/N6847` mapping must not be reintroduced.
...
- Hyundai Complaint business date is `COALESCE(complaint_date, resolving_date, dealer_resolving_date, close_date)`. Production verification found 910 complaint rows with a recoverable business date from 07 Jan 2021 through 21 May 2026; filtering only `complaint_date` incorrectly discarded the Hyundai complaint history.
- Hyundai Operation Wise is monthly/snapshot data. The resolver requires an exact or latest contained `report_period_start/report_period_end` period, deduplicates by `row_hash`, uses the shared DMS code registry, sums `total_count` for WA/WB counts, and sums `total_amt` for VAS/WA/WB amounts. Source-level values remain under MECH and are not assigned to individual advisors.
- Production verification on the 01-19 Jun 2026 Operation Wise snapshot found only five deduplicated source rows: three for Jammu and two for Kathua. All five were unclassified, with source codes `A10AA35100RQ0`, `A10AAOT30GENE`, and `A10AAREP14LNA`; none is a registered VAS, WA, or WB code. The other four locations had no contained snapshot. APIs now return source-row/classified-row/unknown-code diagnostics and warnings so these zeroes are not presented as verified business performance. The cron must reload complete monthly Operation Wise reports for all six locations.
- Canonical RO Billing reconciliation for 01-18 Jun 2026: Akhnoor 6 RO / Rs 22,297.39; Jammu 52 / Rs 491,166.67; Kathua 52 / Rs 243,663.06; RS Pura 10 / Rs 39,801.68; Udhampur 28 / Rs 168,837.20; Vijaypur 51 / Rs 407,714.10; all locations 199 RO / Rs 577,289.41 labour / Rs 796,190.69 parts / Rs 1,373,480.10 revenue.
