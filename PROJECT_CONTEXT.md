...
- Sidebar reads `/api/auth/permissions` and renders denied Purchase Orders, Finance Orders, Business Excellence, and Demo Job Cards entries as locked items with a contact-admin message instead of silently hiding them. AM Kia and AM Hyundai sidebar navigation are department-grouped as `Service`, `Sales`, and `H Promise`; current AM Kia modules live under `Service`, and AM Hyundai mirrors the same department shape. Hyundai Service starts with the live `Business Excellence` section at `/brands/hyundai/business-excellence/overview`; its internal dropdown follows the KIA/Platinum Business Excellence flow inside one shell. Hyundai Overview uses the consolidated `/api/brands/hyundai/business-excellence/overview` endpoint and detailed registers retain their own paginated APIs. Hyundai Business Excellence carries date and location filters in URL state and uses the canonical six-location registry documented in the parity section below. Brand sections and department sections no longer auto-expand by default; users open AM Kia/AM Hyundai manually from the sidebar. Admin-only Access Control appears under Admin Panel only for Super Admin.
- AM Kia includes a direct `Insurance` sidebar link at `/brands/kia/insurance`. It appears alongside Service, Sales, and H Promise but is intentionally not a dropdown. Access is governed by `kia.insurance.view`.
- `npm run build` runs `scripts/clean-next-dev-types.js` through the npm `prebuild` hook before invoking Next.js. This removes only generated `.next/dev` artifacts so an interrupted/corrupted development route declaration cannot break production TypeScript checks.
...
- Hyundai Business Excellence parity uses live endpoints for `/workshop-performance`, `/complaints`, `/performance-intelligence`, and `/ai-summary`. RO Billing and Open RO remain `sourceStatus: live` when their Hyundai tables are available. Workshop Performance derives core tables/KPIs from canonical Hyundai RO Billing and monthly Operation Wise sources. Complaints uses the live call-center source with the verified business-date fallback; unavailable optional sources remain explicitly unavailable and are never replaced by samples.
...
- Correct Hyundai DMS dealer mappings include Jammu `N5203/N5216`, Akhnoor `N5701/N6844`, Kathua `N5804/N6845`, RS Pura `N6815/N6846`, Vijaypur `N6819/N6847`, and Billawar `N6828/N6848`.
...
### AM Hyundai Business Excellence Platinum parity - 20 Jun 2026

- Hyundai Business Excellence uses one canonical six-location dealer registry everywhere: Jammu `N5203/N5216`, Akhnoor `N5701/N6844`, Kathua `N5804/N6845`, RS Pura `N6815/N6846`, Vijaypur `N6819/N6847`, and Billawar `N6826/N6828/N6848`. Database recovery proved that historical Billawar rows use `N6826`; `N6828` remains an accepted legacy alias. Legacy `UDHAMPUR` filter values alias to `BILLAWAR` for saved-link compatibility, while `N5217` and `N6849` remain deliberately unmapped. Legacy `ACTIVE` source rows map to Jammu only when no better dealer field exists.
- Canonical Hyundai RO Billing rules live in `lib/hyundai/business-excellence-calculations.ts`: cancelled `bill_type` rows are excluded; invoice identity is canonical dealer + bill date + bill number; load identity is canonical dealer + RO number; duplicate invoices rank by greatest absolute labour-plus-parts value and then latest upload/id; dashboard revenue is labour plus parts while `total_amt` is reconciliation-only.
- Overview, RO Billing Analysis/Register, Workshop Performance, Open RO, Complaints, Performance Intelligence, freshness, AI cache dependencies, and Service Dashboard use bumped cache namespaces after the parity correction. Their metadata exposes `HYUNDAI_BE_CALCULATION_META`.
- Hyundai Complaint business date is `COALESCE(complaint_date, resolving_date, dealer_resolving_date, close_date)`. Production verification continues to confirm recoverable complaint history beginning 07 Jan 2021; filtering only `complaint_date` incorrectly discards that history.
- Hyundai Operation Wise is monthly/snapshot data with no transaction-level business date. The resolver prefers an exact period end and otherwise uses the latest uploaded snapshot in the selected month, while displaying its actual `report_period_start/report_period_end` coverage. It deduplicates by `row_hash`, uses the shared DMS code registry, sums `total_count` for WA/WB counts, and sums `total_amt` for VAS/WA/WB amounts. Source-level values remain under MECH and are not assigned to individual advisors.
- Production verification on the 01-21 Jun 2026 Operation Wise snapshot found source data for all six locations. Billawar (`N6848`) reports VAS `Rs 66,996.61`, WA `38 / Rs 14,807.39`, and WB `149 / Rs 14,266.12`. APIs preserve the full unclassified-row count but limit the example-code list to 25 values.
- The earlier 01-18 Jun 2026 reconciliation labelled `N6848` as Udhampur; the same data now correctly reports as Billawar. Live verification found no `N5217` or `N6849` rows in the checked core sources.
- Hyundai SOT Analysis is visible for navigation parity but explicitly unavailable. MCP is not used as an SOT substitute.
- The source contract is `docs/hyundai-business-excellence-source-contract.md`. Database indexes are defined in `scripts/setup-hyundai-business-excellence-parity.sql` and were applied with `npm run db:setup-hyundai-be-parity`. Verification commands are `npm run verify:hyundai-be-parity -- YYYY-MM-DD YYYY-MM-DD` and `npm run verify:hyundai-operations -- YYYY-MM-DD`.
- On 24 Jun 2026 the active Hyundai core tables were recovered from the trusted pre-cleanup backups with `npm run db:repair-hyundai-be -- --apply --confirm=REPAIR_HYUNDAI_BE`. Timestamped pre-repair snapshots are `hbe_pre_billing_20260624065722`, `hbe_pre_repair_20260624065722`, and `hbe_pre_operation_20260624065722`.
- Post-recovery integrity totals are RO Billing `134,485`, Repair Order `170,132`, and Operation Wise `34,979`, with one distinct safe SHA-256 per row. Unsupported `JK402/JK501` repair rows were not restored.
- The old hashes reused bill/RO identifiers and discarded legitimate corrections and current rows. Database triggers now normalize dealer aliases and recompute a complete normalized-row SHA-256 before every insert/update, so external crons cannot reintroduce that collision bug. Cron rules are documented in `docs/hyundai-cron-ingestion-contract.md`.
- Recovered 01-24 Jun 2026 canonical RO Billing totals are Jammu `52 JC / Rs 491,166.67`, Akhnoor `6 / Rs 22,297.39`, Kathua `52 / Rs 243,663.06`, RS Pura `10 / Rs 39,801.68`, Vijaypur `51 / Rs 407,714.10`, and Billawar `28 / Rs 168,837.20`; all locations are `199 JC / Rs 1,373,480.10`.
- Hyundai Overview, Workshop, and RO Billing analysis select one complete ranked invoice row ordered by largest absolute labour-plus-parts value, latest upload, then id. They must never independently aggregate labour and parts from different duplicate rows. Incomplete current-period RO Billing coverage displays the loaded CY values and actual through-date but suppresses LY growth and Business Health as not comparable. Open RO/WIP is current-state only and is also not comparable.
