...
- RO Billing analysis cache namespace is `kia:business-excellence:ro-billing:v18`. Business Snapshot overview cache namespace is `kia:business-excellence:overview:v20`. YTD LY comparisons use last year's year-to-date window (`01 Jan LY` through the same CY cutoff date last year), and QTD LY uses last year's quarter-to-date window instead of full prior quarter/year totals. On 01 Jun 2026 the Business Excellence Redis keys were cleared so the next dashboard load fetches fresh Supabase data; the cache system remains enabled and will repopulate after the fresh responses.
- Business Excellence date controls use a narrower single-date calendar container (`max-w-[640px]`) while the compare-date layout keeps the wider two-calendar container (`max-w-[860px]`).
