import { hyundaiComparisonGrowth } from '@/lib/hyundai/business-excellence-calculations'
import {
  emptyHyundaiRoBillingAudit,
  fetchHyundaiRoBillingAudit,
  type HyundaiRoBillingAudit,
} from '@/lib/hyundai/ro-billing-audit'

export type HyundaiComparisonParams = {
  preset: string | null
...
export type HyundaiComparisonRange = {
  startDate: string
  endDate: string
  source: string
}

export type HyundaiRoBillingPeriodMetrics = {
  dedupedJc: number
  labour: number
  parts: number
  revenue: number
...
export type HyundaiCanonicalRoBillingMetrics = {
  sourceAvailable: boolean
  cy: HyundaiRoBillingPeriodMetrics
  ly: HyundaiRoBillingPeriodMetrics
  lyRange: HyundaiComparisonRange
  audit: HyundaiRoBillingAudit
}

export type HyundaiComparisonMetric = {
  cy: number
  ly: number
...
export function resolveHyundaiComparisonRange(
  startDate: string,
  endDate: string,
  comparison: HyundaiComparisonParams,
): HyundaiComparisonRange {
...
export function roBillingDelta(current: number, previous: number, hasSelectedRangeData: boolean) {
  return hasSelectedRangeData ? hyundaiComparisonGrowth(current, previous) : null
}

export function roBillingComparisonLabel(hasSelectedRangeData: boolean) {
  return hasSelectedRangeData ? null : 'No selected-range RO Billing data'
}
...
export async function fetchCanonicalHyundaiRoBillingMetrics({
  cyStart,
  cyEnd,
  lyStart,
  lyEnd,
...
}): Promise<HyundaiCanonicalRoBillingMetrics> {
  const audit = await fetchHyundaiRoBillingAudit(cyStart, cyEnd, dealerCode, {
    lyStartDate: lyStart,
    lyEndDate: lyEnd,
  })

  const emptyPeriod: HyundaiRoBillingPeriodMetrics = {
    dedupedJc: 0,
    labour: 0,
    parts: 0,
    revenue: 0,
  ...
  if (!audit.sourceAvailable) {
    return {
      ...
      audit: emptyHyundaiRoBillingAudit(cyStart, cyEnd, dealerCode, false, {
        lyStartDate: lyStart,
        lyEndDate: lyEnd,
      }),
    }
