...
export type HyundaiRoBillingAuditLevel = 'none' | 'watch' | 'critical'

export type HyundaiRoBillingAudit = {
  ...
  anomaly: {
    level: HyundaiRoBillingAuditLevel
    reasons: string[]
    jcGrowthPct: number | null
    revenueGrowthPct: number | null
...
export type HyundaiRoBillingAuditRangeOptions = {
  lyStartDate?: string
  lyEndDate?: string
}
...
function resolveLyRange(
  startDate: string,
  endDate: string,
  options?: HyundaiRoBillingAuditRangeOptions,
) {
  return {
...
export function emptyHyundaiRoBillingAudit(
  startDate: string,
  endDate: string,
  dealerCode: DealerFilter = null,
  sourceAvailable = false,
  rangeOptions?: HyundaiRoBillingAuditRangeOptions,
): HyundaiRoBillingAudit {
...
function buildAnomaly(current: RoPeriodSummary, ly: RoPeriodSummary, previous: RoPeriodSummary) {
  ...
  let level: HyundaiRoBillingAuditLevel = 'none'

  const addReason = (candidateLevel: HyundaiRoBillingAuditLevel, reason: string) => {
    reasons.push(reason)
    if (candidateLevel === 'critical' || level === 'none') level = candidateLevel
  }

  if (revenueGrowth !== null && revenueGrowth >= 250) addReason('critical', `Revenue is ${revenueGrowth.toFixed(1)}% above LY same period.`)
  else if (revenueGrowth !== null && revenueGrowth >= 100) addReason('watch', `Revenue is ${revenueGrowth.toFixed(1)}% above LY same period.`)

  if (jcGrowth !== null && jcGrowth >= 200) addReason('critical', `JC count is ${jcGrowth.toFixed(1)}% above LY same period.`)
  else if (jcGrowth !== null && jcGrowth >= 100) addReason('watch', `JC count is ${jcGrowth.toFixed(1)}% above LY same period.`)

  if (previousRevenueGrowth !== null && previousRevenueGrowth >= 250) addReason('critical', `Revenue is ${previousRevenueGrowth.toFixed(1)}% above previous same-length period.`)
  else if (previousRevenueGrowth !== null && previousRevenueGrowth >= 100) addReason('watch', `Revenue is ${previousRevenueGrowth.toFixed(1)}% above previous same-length period.`)

  if (previousJcGrowth !== null && previousJcGrowth >= 200) addReason('critical', `JC count is ${previousJcGrowth.toFixed(1)}% above previous same-length period.`)
  else if (previousJcGrowth !== null && previousJcGrowth >= 100) addReason('watch', `JC count is ${previousJcGrowth.toFixed(1)}% above previous same-length period.`)
...
export async function fetchHyundaiRoBillingAudit(
  startDate: string,
  endDate: string,
  dealerCode: DealerFilter = null,
  rangeOptions?: HyundaiRoBillingAuditRangeOptions,
): Promise<HyundaiRoBillingAudit> {
  const { lyStartDate, lyEndDate } = resolveLyRange(startDate, endDate, rangeOptions)
  const dealerKey = dealerCode || 'all'
  const cacheKey = `hyundai:ro-billing-audit:v2:${dealerKey}:${startDate}:${endDate}:${lyStartDate}:${lyEndDate}`

  return getCachedData(
    cacheKey,
    () => queryHyundaiRoBillingAudit(startDate, endDate, dealerCode, rangeOptions),
    CACHE_TTL.DASHBOARD,
  )
}

async function queryHyundaiRoBillingAudit(
  startDate: string,
  endDate: string,
  dealerCode: DealerFilter = null,
  rangeOptions?: HyundaiRoBillingAuditRangeOptions,
  ...
  const result = await analyticsDb.execute(sql`
    ...
    scoped AS (
      SELECT
        ...
        COALESCE(${hyundaiRoBillingDealerSql()}, 'UNMAPPED') AS dealer_code,
    ...
    SELECT
      ...
      )), '{}'::jsonb) FROM summary) AS summaries,
      (SELECT COALESCE(jsonb_agg(jsonb_build_object(
        ...
        'parts', parts,
        'revenue', revenue
      ) ORDER BY bill_date ASC), '[]'::jsonb) FROM daily) AS daily_split,
      (SELECT COALESCE(jsonb_agg(jsonb_build_object(
        ...
        'parts', parts,
        'revenue', revenue
      ) ORDER BY revenue DESC, deduped_jc DESC), '[]'::jsonb) FROM dealers) AS dealer_split,
      (SELECT COALESCE(jsonb_agg(jsonb_build_object(
        'billNo', bill_no,
        'roNo', r_o_no,
        'billDate', bill_date,
  ...
  const summaryFor = (key: string, rangeStart: string, rangeEnd: string): RoPeriodSummary => {
    ...
    return {
      ...
      dedupedJc: numberValue(summary.dedupedJc),
  ...
  return {
    ...
    ly: {
      ...
      parts: ly.parts,
      revenue: ly.revenue,
      jcGrowthPct: growthPct(current.dedupedJc, ly.dedupedJc),
      revenueGrowthPct: growthPct(current.revenue, ly.revenue),
    },
    previousPeriod: {
      startDate: previousStartDate,
      endDate: previousEndDate,
      ...
      jcGrowthPct: growthPct(current.dedupedJc, previous.dedupedJc),
