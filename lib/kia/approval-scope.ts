...
export function isApprovalVisibleTo(appUser: AppUser | null, row: ApprovalScopeRow): boolean {
  ...
  const isBrandWideRole = BRAND_WIDE_APPROVAL_ROLES.has(role) || role.includes('account') || role.includes('finance')
  if (isBrandWideRole && holdsBrandStrictly(appUser, rowBrand)) return true
...
export function applyApprovalBrandDefault<T extends ApprovalScopeRow>(
  ...
  if (!canSeeAllApprovals(appUser) && appUser.role !== 'ceo' && appUser.role !== 'ed') return rows
