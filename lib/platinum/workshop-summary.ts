...
const ALL_DEALER_CODES = ['N5211', 'N6250', 'N6828'] as const
...
export type PlatinumWorkshopSummary = {
...
function dedupCte(start: string, end: string, dealerCode: string | null) {
  return sql`
    raw AS (
      SELECT
        ${platinumRoBillingRoKeySql()} AS jc_key,
        ${serviceCategorySql('work_type')} AS service_category,
...
export async function getPlatinumWorkshopSummary(input: { endDate?: string | null; dealerCode?: string | null }): Promise<PlatinumWorkshopSummary> {
