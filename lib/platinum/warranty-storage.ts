...
export const PLATINUM_WARRANTY_BUCKET = 'platinum-warranty-claims'
...
export async function ensurePlatinumWarrantyBucket() {
  const { data } = await supabaseAdmin.storage.getBucket(PLATINUM_WARRANTY_BUCKET)
  if (data) return
  const { error } = await supabaseAdmin.storage.createBucket(PLATINUM_WARRANTY_BUCKET, {
    public: false,
...
export async function uploadWarrantyEvidence(actionId: string, file: File, index: number) {
  await ensurePlatinumWarrantyBucket()
  const extension = file.type === 'image/png' ? 'png' : file.type === 'image/webp' ? 'webp' : 'jpg'
  const safeName = file.name.replace(/[^a-zA-Z0-9._-]+/g, '-').slice(-80)
  const path = `${actionId}/${Date.now()}-${index}-${safeName || `evidence.${extension}`}`
  const { error } = await supabaseAdmin.storage.from(PLATINUM_WARRANTY_BUCKET).upload(
    path,
...
export async function deleteWarrantyEvidence(paths: string[]) {
  ...
  await supabaseAdmin.storage.from(PLATINUM_WARRANTY_BUCKET).remove(paths)
}

export async function getWarrantyEvidenceUrl(path: string) {
  const { data, error } = await supabaseAdmin.storage
    .from(PLATINUM_WARRANTY_BUCKET)
