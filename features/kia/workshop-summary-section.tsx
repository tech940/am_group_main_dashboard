...
export function WorkshopSummarySection({ endDate, dealerCode }: { endDate?: string | null; dealerCode?: string | null }) {
  const query = useQuery<WorkshopSummary>({
    queryKey: ['kia-workshop-summary', endDate || 'today', dealerCode || 'all'],
    queryFn: async () => {
      const params = new URLSearchParams()
      if (endDate) params.set('endDate', endDate)
      if (dealerCode) params.set('dealer_code', dealerCode)
      const res = await fetch(`/api/brands/kia/business-excellence/workshop-summary?${params.toString()}`, { cache: 'no-store' })
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      {/* Day Wise Trend — the SAME section used on the RO Billing report (includes the MTD target /
      ...
      {/* Location-wise breakdown (no growth contribution) */}
