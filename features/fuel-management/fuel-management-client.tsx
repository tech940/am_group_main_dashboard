      ...
      <div className="fm mx-auto max-w-[1520px] space-y-5 pb-16">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3.5 pt-1">
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-xs font-normal text-slate-500" aria-live="polite">
              {data ? <>Compared with <span className="font-medium text-slate-700">{fmtRange(data.previous.from, data.previous.to)}</span></> : 'Loading period data…'}
              {query.isFetching && data && <span className="ml-2 text-slate-500 font-medium">· Updating…</span>}
            </p>
          </div>
          ...
          <div className="fm-noprint flex flex-wrap items-center gap-2">
            <button
              ...
              className="inline-flex h-9 items-center gap-1.5 rounded-xl border border-slate-200/90 bg-white px-3.5 text-xs font-medium text-slate-700 transition-all hover:bg-slate-50 hover:text-slate-900 shadow-2xs active:scale-98 cursor-pointer"
            >
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-64 rounded-2xl p-1.5 shadow-xl border border-slate-200">
                <DropdownMenuLabel className="text-[11px] font-semibold uppercase tracking-wider text-slate-400 px-2.5 py-1.5">
                  Excel Reports
                </DropdownMenuLabel>
                <DropdownMenuSeparator className="my-1" />
                {FUEL_REPORTS.map((report) => (
                  <DropdownMenuItem key={report.id} className="text-xs font-medium rounded-xl px-2.5 py-2 cursor-pointer" onSelect={() => download(report.id)}>
                    {report.title}
                  </DropdownMenuItem>
                ))}
              </DropdownMenuContent>
            ...
            {tabs.map((item) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'fm-inset-focus inline-flex h-9 items-center gap-2 px-3.5 sm:px-4 rounded-xl text-xs transition-all duration-150 cursor-pointer active:scale-98',
                    active
                ...
                >
                  ...
                  {item.count !== undefined && item.count !== null && item.count > 0 && (
                    <span
                      className={cn(
                        'rounded-full px-1.5 py-0.2 text-[10.5px] font-medium tabular-nums',
