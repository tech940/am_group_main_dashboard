...
import { appendHyundaiDealerCodeParam as appendKiaDealerCodeParam } from '@/lib/hyundai/dealer-branch'
  ...
  const fetchComplaints = useCallback(async () => {
    try {
      ...
      const result = await queryClient.fetchQuery({
        queryKey: ['business-excellence', 'hyundai-complaints', summaryQueryString],
        queryFn: async () => {
          const suffix = summaryQueryString ? `?${summaryQueryString}` : ''
          const response = await fetch(`/api/brands/hyundai/business-excellence/complaints${suffix}`)
          logApiTimings(response, 'hyundai-complaints')
          if (!response.ok) throw new Error('Failed to load Hyundai Complaints dashboard')
      ...
      const [secondaryResult, detailsResult] = await Promise.all([
        queryClient.fetchQuery({
          queryKey: ['business-excellence', 'hyundai-complaints', secondaryQueryString],
          queryFn: async () => {
            const suffix = secondaryQueryString ? `?${secondaryQueryString}` : ''
            const response = await fetch(`/api/brands/hyundai/business-excellence/complaints${suffix}`)
            logApiTimings(response, 'hyundai-complaints-secondary')
            if (!response.ok) throw new Error('Failed to load Hyundai Complaints secondary data')
            return await response.json() as ComplaintResponse
          },
          staleTime: DASHBOARD_STALE_TIME_MS,
        }),
        queryClient.fetchQuery({
          queryKey: ['business-excellence', 'hyundai-complaints', detailsQueryString],
          queryFn: async () => {
            const suffix = detailsQueryString ? `?${detailsQueryString}` : ''
            const response = await fetch(`/api/brands/hyundai/business-excellence/complaints${suffix}`)
            logApiTimings(response, 'hyundai-complaints-details')
            if (!response.ok) throw new Error('Failed to load Hyundai Complaints detail rows')
            return await response.json() as ComplaintResponse
          },
          staleTime: DASHBOARD_STALE_TIME_MS,
        }),
      ...
      console.error('Failed to load Hyundai Complaints dashboard:', error)
    } finally {
      setIsLoading(false)
      setIsDetailLoading(false)
    }
    ...
    return (
      <div className="bg-slate-50 p-8">
        <div className="rounded-[1.5rem] border border-dashed border-slate-200 bg-white p-12 text-center shadow-xl shadow-slate-200/50">
          ...
          <p className="text-sm font-black uppercase tracking-widest text-slate-400">Hyundai Complaints data is unavailable.</p>
        </div>
      </div>
    )
  }
    ...
    <div className="space-y-5 bg-slate-50 p-4 lg:p-6">
      ...
      {showCalendarView && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Hyundai Complaints calendar"
        >
